...
namespace ProjectECommerce.Cache
{
    public class ProductServiceWithCaching : IProductService
    {
        ...
        public Task<CustomResponseDto<List<ProductDto>>> GetProducts()
        {
            var products = _memoryCache.Get<List<Product>>(CacheProductKey).ToList();
            var productsDto = _mapper.Map<List<ProductDto>>(products);

            return Task.FromResult(CustomResponseDto<List<ProductDto>>.Success(200, productsDto));
        }

        public Task<CustomResponseDto<List<ProductDto>>> GetProductsByCategory(string categoryName)
        {
            var products =  _memoryCache.Get<List<Product>>(CacheProductKey).Where(x => x.Category.ToLower() == categoryName.ToLower()).ToList();
            var productsDto = _mapper.Map<List<ProductDto>>(products);
